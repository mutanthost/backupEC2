...
if [ -z "$PRWDIR" ]; then
 if [ `$findprocname | grep "prw.sh r" | grep -v grep | wc -l | tr -d ' '` -gt 0 ]; then
  ...
  if [ $CLUSTERWARE='true' ] && [ -w "$GRIDLOGDIR" ]; then
    PRWDIR=$CWLOGDIR
  else
    PRWDIR=$EXEDIR
  fi
  ...
  if [ ! -f $PRWDIR/PRW_SYS_$HOSTNAME/prwdir ]; then
    if [ ! -d $PRWDIR/PRW_SYS_$HOSTNAME ]; then
      ...
      chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME
    fi
...
defSIDLIST=
# end defaults
for param in EXAMINE_CLUSTER EXAMINE_BG PRWPERM RETENTION WARNINGEMAIL INTERVAL THROTTLE IDLECPU SIDLIST
do
...
defCUSTOMSQL3=
# end defaults
 for param in USE_SQL sessionwait lock latchholder gesenqueue waitchains rmanclient process_memory sqltext ash errorstack sgamemwatch heapdump_level lib_cache_dump_level suspectprocthreshold warningprocthreshold hanganalyze_level systemstate_level CLUSTERPROCS BGPROCS use_gv use_pmap VERSION_10_1 VERSION_10_2 VERSION_11_1 VERSION_11_2 VERSION_12_1 VERSION_12_2 FALL_BACK_TO_OSDEBUGGER STACKCOUNT CUSTOMSQL1 CUSTOMSQL2 CUSTOMSQL3
 do
  if [ `cat $PRWDIR/prwinit.ora | grep $param= | grep -v "#" | wc -l | tr -d ' '` =  1 ]; then
    export $param=`cat $PRWDIR/prwinit.ora | grep $param= | grep -v "#" | cut -d '=' -f2`
  else
...
capturevmstat()
{
  ...
  chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/vmstat
  ...
  do
    ...
    if [ $idle != "id" ]; then
      ...
      chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/idlecpu
    fi
  done
}
...
prwrelocate()
{
  ...
  if [ $CLUSTERWARE = 'true' ] && [ ! -f $PRWDIR/PRW_SYS_$HOSTNAME/prwdir ]; then
    ...
    if [ -w $GRIDLOGDIR ] && [ $PRWDIR != $CWLOGDIR ]; then
      ...
      chmod $PRWPERM $CWLOGDIR/prwinit.ora
      if [ -f $PRWDIR/PRW_SYS_$HOSTNAME/debug ]; then
...
processwarning()
{
 ...
 if [ ! -f $PRWDIR/PRW_SYS_$HOSTNAME/warning_$warningsid ]; then
   ...
   chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/warning_$warningsid
 fi
...
if [ -f $PRWDIR/PRW_SYS_$HOSTNAME/debug ]; then
  ...
  if [ -n "$SIDLIST" ]; then
   ...
   if [ -f $PRWDIR/PRW_SYS_$HOSTNAME/oratab ]; then
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/oratab
   fi
   ...
   ### Get DB version
     ...
     if [ $isinstanceup -gt 0 ]; then
      if [ "$USE_SQL" = 'true' ] || [ "$EXAMINE_BG" = 'true' ]; then
       echo `date`: "Checking DB version for SID $sid"
         ...
         throttlecontrol
         if [ -f $PRWDIR/PRW_SYS_$HOSTNAME/debug ]; then
           nohup ksh -x $EXEDIR/prw.sh sqlstart $PRWDIR/PRW_SYS_$HOSTNAME/SQLvinstance $user $sid $OH 2>&1
         else
           nohup ksh $EXEDIR/prw.sh sqlstart $PRWDIR/PRW_SYS_$HOSTNAME/SQLvinstance $user $sid $OH 2>&1 &
         fi
        ...
        do
         ...
         chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/version_$sid
         if [ `cat $PRWDIR/PRW_SYS_$HOSTNAME/version_$sid | egrep "9.|10.|11.|12." | wc -l` -gt 0 ]; then
           echo `date`: "DB Version for SID $sid is "`cat $PRWDIR/PRW_SYS_$HOSTNAME/version_$sid`
         fi
         if [ `cat $PRWDIR/PRW_SYS_$HOSTNAME/version_$sid | egrep "9.|10.|11.|12." | wc -l` -eq 0 ]; then
     ...
     if [ ! -f $PRWDIR/PRW_SYS_$HOSTNAME/ppids ]; then
       ...
       chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/ppids
     fi
     if [ `echo $sid | cut -c1-3` != 'ASM' ] && [ `echo $sid | cut -c1-3` != 'APX' ] && [ `cat $PRWDIR/PRW_SYS_$HOSTNAME/version_$sid | egrep "9.|10.|11.|12." | wc -l` -gt 0 ] && [ `cat $PRWDIR/PRW_SYS_$HOSTNAME/ppids | grep "SQLprocwatcher_pids$sid " | wc -l | tr -d ' '` -eq 0 ]; then
      throttlecontrol
      if [ -f $PRWDIR/PRW_SYS_$HOSTNAME/debug ]; then
  ...
  chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/sidlist
  echo "$BADSIDLIST" > $PRWDIR/PRW_SYS_$HOSTNAME/badsidlist
  chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/badsidlist
  ...
  do
    if [ ! -d $PRWDIR/PRW_DB_$sid ]
      ...
      chmod $PRWPERM $PRWDIR/PRW_DB_$sid
    ...
    if [ `ps -e -o args | grep "_lmd0" | egrep -v "$badlist" | wc -l | tr -d ' '` -gt 0 ]; then
       ...
       chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/israc_$sid
  done
}
...
### END FUNCTIONS
  ...
  chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/prwinterval

  if [ ! -f $PRWDIR/PRW_SYS_$HOSTNAME/lastvmstat ]; then
    echo "0" > $PRWDIR/PRW_SYS_$HOSTNAME/lastvmstat
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/lastvmstat
  fi
  ...
  chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/*sql
  ...
  chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/examinebg

  echo "true" > $PRWDIR/PRW_SYS_$HOSTNAME/shortstack
  chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/shortstack

  if [ $EXAMINE_CELL = 'true' ]; then
    touch $PRWDIR/PRW_SYS_$HOSTNAME/exadatacells
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/exadatacells
  fi
  ...
  chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/user
  ...
  if [ ! -z "$SIDLIST" ]; then
    ...
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/usersidlist
  fi
 ...
 if [ $EXAMINE_CLUSTER = 'true' ] || [ $EXAMINE_CELL = 'true' ] || [ $FALL_BACK_TO_OSDEBUGGER = 'true' ]; then
  ...
  if [ "$USE_GDB" = 'true' ]
    ...
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/prw_gdb.cmd
  ...
  if [ "$USE_LADEBUG" = 'true' ]
    ...
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/prw_iladebug.cmd
    chmod u+x $PRWDIR/PRW_SYS_$HOSTNAME/prw_iladebug.cmd
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/prw_cladebug.cmd
  ...
  if [ "$USE_DBX" = 'true' ]
    ...
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/prw_dbx.cmd
  ...
  if [ $EXAMINE_CLUSTER = 'true' ]
    ...
    if [ ! -d $PRWDIR/PRW_CLUSTER ]
      ...
      chmod $PRWPERM $PRWDIR/PRW_CLUSTER
    fi
  fi
  ...
  while [ 1 -ne 0 ]; do
   ...
   if [ $EXAMINE_CLUSTER = 'true' ]
    then
     ...
     if [ ! -f $PRWDIR/PRW_CLUSTER/prw_"$HOSTNAME"_crsstat_`date +"%m-%d-%y"`.out ]; then
       ...
       chmod $PRWPERM $PRWDIR/PRW_CLUSTER/prw_"$HOSTNAME"_crsstat_`date +"%m-%d-%y"`.out
     fi
   ...
   then
     ...
     do
       ...
       if [ ! -f $PRWDIR/PRW_CELL/pw_"$HOSTNAME"_"$cellgather"_`date +"%m-%d-%y"`.out ]; then
         ...
         chmod $PRWPERM $PRWDIR/PRW_CELL/pw_"$HOSTNAME"_"$cellgather"_`date +"%m-%d-%y"`.out
       fi
   ...
   for sqlsid in `cat $PRWDIR/PRW_SYS_$HOSTNAME/sidlist | tr "|" " "`
     ...
     then
       ...
       if [ `egrep "9.|10.|11.|12." $PRWDIR/PRW_SYS_$HOSTNAME/version_$sqlsid | wc -l` -gt 0 ]; then
       ...
       if [ `cat $PRWDIR/PRW_SYS_$HOSTNAME/israc_$sqlsid` = true ] && [ ! -f $PRWDIR/PRW_SYS_$HOSTNAME/viewtype_$sqlsid ] && [ "$use_gv" != "n" ]; then
         ...
         chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/viewtype_$sqlsid
       else
         echo "v" > $PRWDIR/PRW_SYS_$HOSTNAME/viewtype_$sqlsid
         chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/viewtype_$sqlsid
       fi
      ...
      if [ `ls -l $PRWDIR/PRW_SYS_$HOSTNAME | grep waitchains | grep $sqlsid | grep out | wc -l | tr -d ' '` -gt 0 ]; then
       ...
       do
         ...
         if [ "$sqlname" = 'MEMlru' ] && [ "$MEMlru" != 'y' ]; then
           ...
           continue
         elif [ "$sqlname" = 'pgamemory' ] && [ "$pgamemory" != 'y' ]; then
           continue
       ...
       if [ "$sgamemwatch" = 'avoid4031' ] || [ "$sgamemwatch" = 'diag' ]; then
        ...
        if [ "$memissue" -eq 1 ]; then
          ...
          if [ -n "$WARNINGEMAIL" ]; then
            ...
            chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/spwarningfile
            if [ "$sgamemwatch" = 'avoid4031' ]; then
...
DBEOF
       ...
       if [ "$pgamemory" = 'y' ]; then
         ...
         echo `date`": Free Memory is: $mysum MB"
         if [ $mysum -lt 1000 ]; then
           echo `date`": WARNING: Memory is low..."
           LOWMEM=y
         else
           LOWMEM=n
         fi
      ...
      if [ $platform = 'Linux' ] && [ -x /usr/bin/top ]; then
       ...
       chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/top$sqlsid.out
       for i in $pidlist
       do
         if [ `cat $PRWDIR/PRW_SYS_$HOSTNAME/top$sqlsid.out | grep $i | awk '{print $9}' | cut -d "." -f1 | wc -l | tr -d ' '` -eq 0 ]; then
           ...
           if [ -n $pidcpu ]; then
            if [ $pidcpu -gt 90 ]; then
             ...
             chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/listSQLtop$sqlsid
            fi
           fi
         fi
       done
        ...
        if [ `sort -mu $PRWDIR/PRW_SYS_$HOSTNAME/listSQL*$sqlsid | wc -l` -gt 0 ]; then
         ...
         if [ $numaddprocs -ge $warningprocthreshold ]; then
           ...
           if [ -n "$WARNINGEMAIL" ]; then
             ...
             chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/wcwarningfile
           fi
         fi
         ...
         if [ $numaddprocs -ge $warningprocthreshold ]; then
          ...
          if [ ! -f $PRWDIR/PRW_SYS_$HOSTNAME/hang_$sqlsid_$DATAHOUR ]; then
            ...
            chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/hang_$sqlsid_$DATAHOUR
          else
          ...
          if [ ! -f $PRWDIR/PRW_SYS_$HOSTNAME/wcwarning_$sqlsid ]; then
            ...
            chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/wcwarning_$sqlsid
          fi
...
DBEOF
     ...
     if [ ! -f $PRWDIR/PRW_SYS_$HOSTNAME/numaddprocs_$sqlsid ]; then
       ...
       chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/numaddprocs_$sqlsid
     fi
   ...
   if [ `ls -l $PRWDIR/PRW_SYS_$HOSTNAME | grep list | egrep -v "sidlist|proclist" | wc -l | tr -d ' '` -gt 0 ]; then
     ...
     chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/proclist
   fi
   if [ -f $PRWDIR/PRW_SYS_$HOSTNAME/proclist ]; then
   ...
   && [ $totalprocs -gt 0 ]; then
     ...
     chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/opidlist
    if [ `grep ASM $PRWDIR/PRW_SYS_$HOSTNAME/sidlist | wc -l` -gt 0 ]; then
     ...
     chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/asmpidlist
    fi
   fi
...
and p.spid in (select prwpid from procwatcher_pids)
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLvprocess_memory.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLASMvprocess_memory.sql
chmod u+x $PRWDIR/PRW_SYS_$HOSTNAME/SQLASMvprocess_memory.sql
   ...
   for proc in `cat $PRWDIR/PRW_SYS_$HOSTNAME/proclist`
     ...
     if [ "$pgamemory" = 'y' ] && [ $proctype = 'oracle' ] && [ "$LOWMEM" = 'y' ]; then
...
'shortstack')
  ...
  buildshortstackscript()
  {
    ...
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/ss_$proc.sql
...
DBEOF
  ...
  'test')
    ...
    && [ `ps -e -o pid,args | grep "_pmon" | grep $SID | grep "shortstack run" | wc -l | tr -d ' '` -eq 0 ]; then
      ...
      chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/shortstack_$SID_test
...
'oradebug')
  ...
  buildoradebugscript()
  {
    ...
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/oradebug_$proc.sql
...
'start')
  ...
  if [ "$2" = 'debug' ]; then
      ...
      chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/debug
  fi

  ### Trace mode? (linux only right now)
  if [ "$3" = 'strace' ]; then
      echo "strace" > $PRWDIR/PRW_SYS_$HOSTNAME/strace
      chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/strace
  fi
  if [ "$3" = 'stack' ]; then
      echo "stack" > $PRWDIR/PRW_SYS_$HOSTNAME/stack
      chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/stack
  fi
    ...
    chmod $PRWPERM $PRWDIR/prw_$HOSTNAME.log
  ...
  while [ 1 -ne 0 ]; do
    ...
    restartprw()
    {
      ...
      if [ $BOOTDIFF -gt 3600 ]; then
        ...
        chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/lastreboot
...
'sqlstart')
  ...
  if [ ! -f $PRWDIR/PRW_SYS_$HOSTNAME/disabled_SQL_$SID$sqlname ]; then
    ...
    chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/disabled_SQL_$SID$sqlname
  else
...
#######################################################################
" > $PRWDIR/prwinit.ora
chmod $PRWPERM $PRWDIR/prwinit.ora
...
$ECHO "
DROP DIRECTORY prwsysdir;
set timing on
drop table procwatcher_pids; " > $PRWDIR/PRW_SYS_$HOSTNAME/SQLdrop_procwatcher_pids.sql
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLdrop_procwatcher_pids.sql
chmod u+x $PRWDIR/PRW_SYS_$HOSTNAME/SQLdrop_procwatcher_pids.sql

$ECHO "
CREATE OR REPLACE DIRECTORY prwsysdir as '$PRWDIR/PRW_SYS_$HOSTNAME';
set timing on
CREATE TABLE procwatcher_pids
                   (prwpid       NUMBER(10))
     ORGANIZATION EXTERNAL
     (
       TYPE ORACLE_LOADER
       DEFAULT DIRECTORY prwsysdir
       ACCESS PARAMETERS
       (
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLprocwatcher_pids.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLgvsessionwait.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLvsessionwait.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLgvlock.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLvlock.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLgvlatchholder.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLvlatchholder.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLvinstance.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLMEMsgastat.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLMEMheapdetails.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLMEMsgadynamic.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLMEMtop20sqls.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLMEMlru.sql
chmod u+x $PRWDIR/PRW_SYS_$HOSTNAME/SQLMEMlru.sql

$ECHO "set timing on
set echo on" > $PRWDIR/PRW_SYS_$HOSTNAME/SQLMEMflush.sql
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLMEMflush.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLhang.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLRAChang.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLv111waitchains.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLvwaitchains.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLrmanclient.sql
...
column PROC format a15 tru
SELECT * from (
SELECT 'PROC '||spid PROC, 'INST '||i.instance_name INST, username, pga_used_mem, pga_alloc_mem
FROM v\$process p, v\$instance i
WHERE pga_used_mem > (900*1024*1024)
ORDER BY pga_used_mem desc)
where rownum < 6;" > $PRWDIR/PRW_SYS_$HOSTNAME/SQLpgamemory.sql
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLpgamemory.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLgvgesenqueue.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLvgesenqueue.sql
...
spid in
(select prwpid from procwatcher_pids);" | sed 's@,)@)@' > $PRWDIR/PRW_SYS_$HOSTNAME/SQLvProcess.sql
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLvProcess.sql
...
and p.spid in
(select prwpid from procwatcher_pids);" | sed 's@,)@)@' > $PRWDIR/PRW_SYS_$HOSTNAME/SQLsqltext.sql
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLsqltext.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLash.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLASMvProcess.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLASMsqltext.sql
...
chmod $PRWPERM $PRWDIR/PRW_SYS_$HOSTNAME/SQLASMash.sql
