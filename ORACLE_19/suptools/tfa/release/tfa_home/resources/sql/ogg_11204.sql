Rem
Rem $Header: tfa/src/v2/tfa_home/resources/sql/ogg_11204.sql /main/1 2018/05/28 15:06:26 bburton Exp $
Rem
Rem ogg_11204.sql
Rem
...
Rem
Rem    NAME
Rem      ogg_11204.sql - <one-line expansion of the name>
Rem
Rem    DESCRIPTION
...
Rem    SQL_SOURCE_FILE: tfa/src/v2/tfa_home/resources/sql/ogg_11204.sql
...
Rem    xiaodowu    01/25/18 - Modified original script to 1) remove "connect /
Rem                           as sysdba" and 2) rename create/replace objects
Rem                           with oracle_tfa prefix
Rem    xiaodowu    01/25/18 - Created
Rem
...
define hcversion = 'V2.1.42';
define SRDCNAME='GoldenGate_HealthCheck'
set truncate off
set numwidth 15
...
prompt Analysis: <a href="#History">History</a>   <a href="#Notification">Notifications</a>   <a href="#DBA OBJECTS">Objects</a> <a href="#Configuration checks">Checks</a>   <a href="#Performance Checks">Performance</a>   <a href="#Wait Analysis">  Wait Analysis </a> <a href="#Topology"> Topology </a>    <a href="#AlertLogDetails">AlertLog Details</a> 
...
prompt
set feedback on
select component_name, count(*) "Open Transactions",sum(cumulative_message_count) "Total LCRs" from gv$Goldengate_transaction where component_type='APPLY' group by component_name order by 3,1;
...
dbpsu      boolean;
exadatapsu boolean;
cursor   reghist is select * from dba_registry_history where action_time=(select max(action_time) from dba_registry_history where namespace='SERVER' and action='APPLY');
...
--  check for capture cloning fix in bp3   
   select count(*) into countr from dba_procedures where owner='SYS' and object_name='DBMS_XSTREAM_GG_ADM' and procedure_name='CREATE_CAPTURE';
   if countr>0 then
       bpvalue  := 'BP3' ;
   end if;
...
--  check for batchsql_mode fix for bp2
...
create or replace function oracle_tfa_get_parameter(
...
begin
  ...
  return TRUE;
end oracle_tfa_get_parameter;
/

create or replace procedure oracle_tfa_verify_init_parameter( 
  param_name         IN varchar2, 
...
begin
  ...
  -- Get value
  if oracle_tfa_get_parameter(param_name, current_value, 'v$parameter', 'name', 'value') = FALSE 
...
end oracle_tfa_verify_init_parameter;
/
...
begin
    execute immediate 'create table oracle_tfa_no_heartbeat (' ||
...
begin
  select owner||'.'||object_name into :v_table_name from
    ...
    :v_table_name :='oracle_tfa_no_heartbeat';
end;
/
...
prompt +  This is an indicator about whether the post_install instructions for OGG/RDBMS patch have been applied.
...
prompt
select distinct(obj.timestamp) hide, reg.ACTION Action, reg.ID ID, reg.COMMENTS Comments, to_char(reg.action_time,'YYYY-MM-DD:HH24:MI:SS') latest_time,
    ...
    where reg.action_time = (select max(action_time) from dba_registry_history) and
 ...
 --- capture state has not changed for at least 3 minutes 
  ...
  -- Check for too many registered archive logs
    ...
    if failed then
      ...
      dbms_output.put_line('+    altering the checkpoint_retention_time capture parameter to a different value  by including the following line in the extract parameter file ');
      dbms_output.put_line('    TRANLOGOPTIONS CHECKPOINTRETENTIONTIME number_of_days  ');
...
begin
  -- Error checks first
       ...
       oracle_tfa_verify_init_parameter('enable_goldengate_replication', 'TRUE', verbose, 
                        '+    To use Oracle GoldenGate with Oracle Database 12g Release 1 (12.1.0.2 or up), '||  
                      'this parameter should be set to TRUE in the init.ora or spfile',
                      '+    This parameter can be set dynamically using: ALTER SYSTEM set enable_goldengate_replication=TRUE;',
                     is_error=>true);
  

       select count(*) into compcnt from v$parameter where name='_lmn_compression';
       if compcnt >0 then
           oracle_tfa_verify_init_parameter('_lmn_compression', 'TRUE', verbose, 
                        '+    This parameter should be removed from the init.ora or spfile',
                        '+    However, the parameter can be set dynamically using: ALTER SYSTEM set "_lmn_compression"=TRUE; so that compressed tables can be captured temporarily without restarting the database',is_error=>true);
       end if;

  
  -- Then warnings

         oracle_tfa_verify_init_parameter('compatible','11.2.0.4', verbose,
                        '+    To use full features of GoldenGate 12c Integrated Capture (including native DDL (triggerless DDL), '||  
...
-- explictly check if aq_tm_processes has been manually set to 10.  If so, raise error.
 ...
 begin
  ...
  oracle_tfa_verify_init_parameter('streams_pool_size', '0', TRUE, 
                        '+    If this parameter is 0 and sga_target is non-zero, then autotuning of the streams pool is implied. It is recommended that streams_pool_size be set to a non-zero value (at least 1000M)'||newline||
                        ...
                        '+    If sga_target is 0, the minimum recommendation for streams_pool_size is 1000M.'|| newline||
...
cursor lrg_txn_running is select s.apply_name,server_id,state, message_sequence from v$gg_apply_server s, dba_apply_parameters p where
                 s.apply_name = p.apply_name 
                 and p.parameter='EAGER_SIZE'
                 and s.state not in ('IDLE','INACTIVE') 
                 and s.message_sequence >= p.value;
cursor no_batchsql is select apply_name from dba_apply where purpose='GoldenGate Apply' MINUS (select apply_name from dba_apply_parameters where parameter='_DML_REORDER');

 BEGIN
     select count(*) into crow_count from dba_capture c where c.purpose = 'GoldenGate Capture';
     select count(*) into arow_count from dba_apply where  purpose = 'GoldenGate Apply';
	 ...
	 dbms_output.put_line('+  <b>INFO</b>:  Verify that database parameter streams_pool_size is set appropriately ');
      select count(*) into target_set from v$parameter where name in ( 'sga_target','memory_target') and value != '0';
       ...
       if arow_count > 0 then
         ...
         dbms_output.put_line('+') ;
         select count(*) into rcount from dba_tables where table_name='STREAMS$_APPLY_PROGRESS' and owner = 'SYS' and ini_trans=1;
         if rcount > 0 then
...
select p.apply_name, e.source_database, utl_raw.cast_to_varchar2(e.external_source_pos)  source_commit_scn,
       ...
       sys.lcr$_row_record.get_commit_scn_from_position(error_pos) error_scn,
...
--prompt     List capture restart info over last 10 minutes
--SELECT
...
-- prompt     List apply restart info over last 10 minutes
-- SELECT
--       apply_name, status, status_change_time,error_number, error_message,
--       start_date,end_date,
--       versions_startscn, versions_starttime,
...
prompt   ordered by thread#, sequence#, first_change#
select * from v$standby_log order by thread#,sequence#,first_change#;
prompt
prompt   ordered by first_change#, thread#, sequence#
...
REM   Select inst_id,APPLY_NAME, total_messages_dequeued, total_messages_spilled,         Elapsed_dequeue_time Elapsed_dequeue, 
...
SELECT c.inst_id,ap.APPLY_NAME,
...
SELECT inst_id, APPLY_NAME,      TOTAL_RECEIVED,TOTAL_ASSIGNED,TOTAL_APPLIED,
     STARTUP_TIME,
...
prompt
prompt +**   Detail    **+
select * from gv$goldengate_transaction where component_type='APPLY' order by component_name,first_message_position;
prompt
...
prompt
SELECT a.inst_id,ap.APPLY_NAME,
       a.server_id SRVR,
       a.STATE,
       a.TOTAL_ASSIGNED ASSIGNED,
       a.TOTAL_MESSAGES_APPLIED msg_APPLIED,
       ...
       WHERE a.server_id = 0 and 
             a.APPLY_NAME = ap.APPLY_NAME order by a.inst_id,a.apply_name, a.server_id;
prompt
prompt <a href="#Summary">Return to Summary</a>
...
prompt     Using V$SQL
select a.inst_id, a.apply_name,  a.server_id, a.state, a.total_messages_applied,
...
FROM gV$GG_APPLY_SERVER a, gV$SESSION s
WHERE a.SID = s.SID AND
a.SERIAL# = s.SERIAL#  AND a.inst_id = s.inst_id 
order by a.apply_name,a.server_id;
prompt
prompt <a href="#Summary">Return to Summary</a>
...
prompt    Apply server transactions ordered by source commit_position and dependent transaction scns.
prompt
select a.inst_id, a.APPLY_NAME, 
   SUBSTR(s.PROGRAM,INSTR(S.PROGRAM,'(')+1,4) PROCESS_NAME,
   server_id SRVR,a.state,
...
, statistic_time, statistic_name
,  CASE WHEN REGEXP_LIKE(statistic_value, '^[[:digit:]]*.?[[:digit:]]?$') THEN to_char(round(to_number(statistic_value),3))
