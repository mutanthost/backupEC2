Rem
Rem $Header: tfa/src/v2/tfa_home/resources/sql/srdc_dbupgdiag.sql /main/1 2017/04/20 22:18:55 bburton Exp $
Rem
Rem srdc_dbupgdiag.sql
Rem
Rem Copyright (c) 2017, Oracle and/or its affiliates. All rights reserved.
Rem
Rem    NAME
Rem      srdc_dbupgdiag.sql - <one-line expansion of the name>
Rem
Rem    DESCRIPTION
Rem    This script is intended to provide a user friendly output to diagonise
Rem    the status of the database before (or) after upgrade.
Rem    This script does not make any DDL / DML modifications.
Rem    
Rem    Taken from dbupgdiag.sql originally provided by Oracel Support
Rem
Rem    Original Authors:  
Rem    Raja Ganesh and Agrim Pandit - Oracle Support Services - DataServer Group
Rem
Rem    NOTES
Rem      <other useful comments, qualifications, etc.>
Rem
Rem    BEGIN SQL_FILE_METADATA
Rem    SQL_SOURCE_FILE: tfa/src/v2/tfa_home/resources/sql/srdc_dbupgdiag.sql
...
Rem    bburton     03/15/17 - Fix spooling
Rem    bburton     03/01/17 - SQL to collect data for DB upgrade issues -
Rem                           requires to run as sysdba
Rem    bburton     03/01/17 - Created
Rem
...
SELECT to_char(sysdate,'dd_Mon_yyyy_hhmi') timecol,'.log' spool_extension FROM sys.dual;
column output new_value dbname
SELECT value || '_' output FROM v$parameter WHERE name = 'db_name';
spool db_upg_diag_&&dbname&&timestamp&&suffix
set linesize 150
set pages 100
set trim on
set trims on
