Rem
Rem $Header: tfa/src/v2/tfa_home/resources/sql/srdc_exp_performance.sql /main/1 2018/05/28 15:06:27 bburton Exp $
Rem
Rem srdc_exp_performance.sql
Rem
...
Rem
Rem    DESCRIPTION
Rem      Extract information for performance related EXP issues
Rem
Rem    NOTES
Rem      None
Rem
Rem    BEGIN SQL_FILE_METADATA
Rem    SQL_SOURCE_FILE: tfa/src/v2/tfa_home/resources/sql/srdc_exp_performance.sql
...
REM srdc_exp_performance.sql - Gather Information for EXP Performance Issues
define SRDCNAME='EXP_PERFORMANCE'
...
SELECT '=================================================================================================================================' FROM dual
UNION ALL
SELECT 'All wait events and time in wait for the active export process(es):' FROM dual
UNION ALL
SELECT '=================================================================================================================================' FROM dual;
SET HEADING ON
...
and session_id in (select sid from v$session where UPPER(program) LIKE 'EXP%')
...
execute immediate stmt into v_long_op_flag;
  if (v_long_op_flag > 0 ) then      
     ...
     for longop in (select sid,target,opname, sum(totalwork) totwork, sum(sofar) sofar, sum(totalwork-sofar) blk_remain, Round(sum(time_remaining/60),2) time_remain                        
      from v$session_longops where sid in (select sid from v$session where UPPER(program) LIKE 'EXP%') and opname NOT LIKE '%aggregate%' and totalwork <> sofar group by sid,target,opname)                        
     loop  
