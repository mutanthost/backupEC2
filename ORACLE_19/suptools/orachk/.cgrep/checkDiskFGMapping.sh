#!/bin/sh
#
# $Header: tfa/src/orachk/src/checkDiskFGMapping.sh /main/14 2018/04/24 23:45:26 rojuyal Exp $
#
...
#
#    MODIFIED   (MM/DD/YY)
#    rojuyal     05/05/15 - BUG 20686530
#    rojuyal     11/28/14 - Handle cell ips and names , flash disk support
#    rojuyal	 08/01/14 - Handle Root user 
#    rojuyal     08/25/14 - Fix for bugs:19491909,19049790,18808667,19492031
#    cgirdhar    05/27/14 - Fix for bug 18808667
...
do
  ...
  case $arg in
    ...
    *)
      ...
      echo "Usage: $pname [-exachk]"
      echo "	      -exachk    : Don't prompt user for input and don't take corrective action. Just check for disks in wrong failgroup and report."
      echo ""
...
if [[ $no_args -eq "1" ]]; then 
  echo "Command line arg missing. Exiting."
  echo ""
  echo "Usage: $pname [-exachk]"
  echo "	      -exachk    : Don't prompt user for input and don't take corrective action. Just check for disks in wrong failgroup and report."
  echo ""
...
if [[ -n "$TMP_OUTPUT" ]]; then 
  RTEMPDIR=$TMP_OUTPUT
  if [[ "$usern" != "root" ]]; then
    if [[ ! -d $RTEMPDIR/$usern ]]; then 
      mkdir $RTEMPDIR/$usern >/dev/null 2>&1
    fi
    cp $RTEMPDIR/cells.out $RTEMPDIR/$usern/cells.out
    RTEMPDIR=$RTEMPDIR/$usern;
  fi
fi

oh=$ORACLE_HOME
workdir=$RTEMPDIR/checkDiskFGMapping
reportfile=${workdir}/checkDiskFGMapping_REPORT.txt
logfile=${workdir}/checkDiskFGMapping.log
exachk_hosts_file=$RTEMPDIR/cells.out
exachk_all_cells=/etc/oracle/cell/network-config/cellip.ora
...
if [ ! -e "${dcli_cmd}" ]; then dcli_cmd='/usr/bin/dcli'; fi

for icellname in `cat $exachk_all_cells|cut -d= -f2|cut -d\; -f2|sed 's/"//g'`
do
  CELL_ERROR=0
  if [[ `grep -wic "$icellname" "$exachk_hosts_file"` -eq 0 ]]
  then
    VAR_TMP="${icellname//[^.]}"
    VAR_VAL=$(echo "$icellname"|sed 's/\.//g') 

    if [[ ${#VAR_TMP} -eq 3 ]]; then
      if expr "$VAR_VAL" : '-\?[0-9]\+$' >/dev/null; then
	t_cellname=$(grep -w "$icellname" /etc/hosts|grep -v "^ *#"|awk '{print $NF}'|cut -d'-' -f1|head -1)
        if [[ `grep -wic "$t_cellname" "$exachk_hosts_file"` -eq 0 ]]; then
          CELL_ERROR=1
	fi 
      else
        CELL_ERROR=1
    ...
    if [[ $CELL_ERROR -eq 1 ]]; then
      echo "All storage servers should be online and accessible" | tee -a $logfile
      echo ""
      echo "Following Storage servers are accessible:" | tee -a $logfile
      cat "$exachk_hosts_file"|cut -d= -f2|sed 's/ //g'| tee -a $logfile
      exit 1
    fi
  fi
done
...
findDisksInWrongFG()
{
  ...
  ${oh}/bin/kfod disks=all a='o/*/*' > /dev/null
  if [ $? -eq 0 ] ;
  then
    ${oh}/bin/kfod disks=all a='o/*/*' | grep "o/" | awk '{print $4}' > $disks_file
  else
    ${oh}/bin/kfod disks=all asm_='o/*/*' | grep "o/" | awk '{print $4}' > $disks_file
  fi
  ...
  do
    ...
    if [ `echo "$cellip" |grep -ci ";"` -gt 0 ]; then cellip=$(echo "$cellip"|cut -d';' -f2);fi

    if [ $exachk -eq 0 ]
    then
      cellname=`${dcli_cmd} -l ${dcli_user} -c "$cellip" hostname -s | awk -F' ' '{ print $2 }'`
    else
      cellname=`grep "$cellip" $exachk_hosts_file | awk '{print $3}'`
      if [[ -z $cellname ]]; then
	cellname=$(grep -w "$cellip" /etc/hosts|grep -v "^ *#"|awk '{print $NF}'|cut -d'-' -f1|head -1)
      fi
    ...
    tmp_cellname=$(echo "$cellname"| awk '{ print tolower($0) }'| sed 's/[^a-z0-9]//g');
    ...
    then
      tmp_gname_suffix=$(echo "$gname_suffix"| awk '{ print tolower($0) }'| sed 's/[^a-z0-9]//g');

      if [ "`echo "$tmp_gname_suffix" | awk '{ print tolower($0) }'`" != "`echo "$tmp_cellname" | awk '{ print tolower($0) }'`" ]
      then
        tmp_gname=$(echo "$gname"| awk '{ print tolower($0) }'| sed 's/[^a-z0-9]//g');
        if [[ `echo "$tmp_gname" | grep -ic "${tmp_cellname}$"` -eq 0 ]]
        then
    ...
    gi_version=$(echo -e "set heading off feedback off timing off \nselect substr(version,0,4) from v\$instance;"|$ORACLE_HOME/bin/sqlplus -s "/ as sysdba"|grep -v ^$|sed 's/\.//g')
    if [ $gi_version -ge 122 ]
    then
        ${oh}/bin/kfed dev=$path op=read cnt=1 blknum=0 blksz=4096 > $disk_header_file
    else
    ...
    fgname_nsc=$(echo "$fgname"| awk '{ print tolower($0) }'| sed 's/[^a-z0-9]//g');

    tmp_fgname=$fgname
    if [ "$fgname_nsc" == "$tmp_cellname" ]
    then
	tmp_fgname=$cellname
    fi
    ...
    if [ $exachk -eq 0 ]
    then
      hname=`${dcli_cmd} -l ${dcli_user} -c $tmp_fgname hostname -s | awk -F' ' '{ print $2 }'`
    else
      tolower_wrapper $tmp_fgname
    ...
    if [ "$fgname_nsc" == "$tmp_cellname" ]
    then
	hname_nsc=$(echo "$hname"| awk '{ print tolower($0) }'| sed 's/[^a-z0-9]//g');
    	cmp_case_insensitive $fgname_nsc $hname_nsc
    else
    ...
    if [[ `echo "$gname"|grep -c '_CD'` -gt 0 ]]; then
      dgname_from_gname=`echo $gname | sed 's/_CD.*$//g'`;
    elif [[ `echo "$gname"|grep -c '_FD'` -gt 0 ]]; then
      dgname_from_gname=`echo $gname | sed 's/_FD.*$//g'`;
    fi
...
dropOrAddDisks()
{
  ...
  do
    ...
    num_zeroes=`sqlplus / as sysdba < $tmpsql | egrep -A 2 -i "rebal_inprogress|dg_not_mounted"  | egrep "0" | wc -l`
...
#    rm -rf $tmpspool
    ...
    then
      ...
      echo "Running $tmpsql with output spooled to $tmpspool" | tee -a $logfile
      sqlplus / as sysdba < $tmpsql >& /dev/null
#      rm -rf $tmpsql
#      rm -rf $tmpspool
    fi
