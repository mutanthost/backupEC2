Rem $Header: tfa/src/orachk/src/utluppkg.sql /main/3 2017/10/23 07:40:15 cgirdhar Exp $
Rem
Rem utluppkg.sql
Rem
...
Rem    apfwkr      07/05/17 - Backport fvallin_bug-26091180 from main c_build=017
Rem    bymotta     07/07/17 - BLR BACKPORT OF BUG 24923215 ON TOP OF 12.1.0.2.0
Rem                           (BLR #5871784) - Concurrent Statistics is ON and
Rem                           RESOURCE MANAGER is OFF which is an invalid
Rem                           configuration. --c_build = 016
Rem    hvieyra     10/05/16 - up c_build to 15 for txn  hvieyra_bug-20950535 
Rem    hvieyra     01/25/17 - Backport hvieyra_bug-20950535
Rem    cmlim       10/05/16 - up c_build to 14 for txn cml_bug_24696626_mos:
Rem                         - bug 24696626: do not in 12.1 recommend
Rem                           pga_aggregate_limit at all.
Rem                         - bug 3596360: pdbs_para cannot be 0.
Rem    apfwkr      10/14/16 - Backport apfwkr_blr_backport_20591183_12.1.0.2.0
Rem                           from st_rdbms_12.1
Rem    apfwkr      03/01/16 - Backport frealvar_ncaclf from
Rem                           st_rdbms_12.1.0.2.0dbbp
Rem    apfwkr      02/25/16 - Backport
Rem                           apfwkr_ci_backport_16439813_12.1.0.2.160419dbpsu
Rem                           from st_rdbms_12.1.0.2.0dbpsu
Rem    apfwkr      04/07/16 - Backport
Rem                           apfwkr_ci_backport_22528741_12.1.0.2.160419dbpsu
...
Rem    apfwkr      01/22/16 - Backport cmlim_bug-22528741_12102psu from
Rem    cmlim       01/20/16 - bug 22528741: memory sizing for upgrade to 12c
Rem                         - copy build #11 file from MOS over c_build 6 here
Rem                         - update c_build from 11 to 12
Rem    apfwkr      01/19/16 - Backport frealvar_lad1_in_use from
Rem    frealvar    01/15/15 - Bug 16439813 log_archive_dest_1 as standby service
Rem                           st_rdbms_12.1.0.2.0dbbp
...
Rem    apfwkr      04/24/15 - Backport welin_bug-20591183 from main
...
c_build       CONSTANT VARCHAR2(30)    := '017';
...
FUNCTION get_npdbs     RETURN NUMBER;   -- get total # of pdbs in cdb
...
-- jvm_mitigation patch check backport for bug 20950535. Check if JAVAVM mitigation patch is installed in the database.
FUNCTION  jvm_mitigation_patch_check (result_txt OUT VARCHAR2) RETURN number;
PROCEDURE jvm_mitigation_patch_fixup;
FUNCTION  jvm_mitigation_patch_gethelp (HelpType IN NUMBER) RETURN VARCHAR2;
FUNCTION  jvm_mitigation_patch_fixup (result_txt IN OUT VARCHAR2, pSqlcode IN OUT NUMBER) RETURN number;

-- post_jvm_mitigat patch check backport for bug 20950535. Information to warn customer at tend of upgrade.
FUNCTION  post_jvm_mitigat_patch_check (result_txt OUT VARCHAR2) RETURN number;
PROCEDURE post_jvm_mitigat_patch_fixup;
FUNCTION  post_jvm_mitigat_patch_gethelp (HelpType IN NUMBER) RETURN VARCHAR2;
FUNCTION  post_jvm_mitigat_patch_fixup (result_txt IN OUT VARCHAR2, pSqlcode IN OUT NUMBER) RETURN number;
...
--  
FUNCTION  conc_res_mgr_check (result_txt OUT VARCHAR2) RETURN number;
FUNCTION  conc_res_mgr_gethelp (HelpType IN NUMBER) RETURN VARCHAR2;
PROCEDURE conc_res_mgr_fixup;
FUNCTION  conc_res_mgr_fixup (result_txt IN OUT VARCHAR2, pSqlcode IN OUT NUMBER) RETURN number;

-- BUG 26091180 - ADD IN PREUPGRADE TOOL:UTLRP FAILS TO VALIDATE APEX 5.0.1, 5.0.2 and 5.0.3 
--  
FUNCTION  apex_version_5_check (result_txt OUT VARCHAR2) RETURN number;
FUNCTION  apex_version_5_gethelp (HelpType IN NUMBER) RETURN VARCHAR2;
PROCEDURE apex_version_5_fixup;
FUNCTION  apex_version_5_fixup (result_txt IN OUT VARCHAR2, pSqlcode IN OUT NUMBER) RETURN number;
...
max_minvp    BINARY_INTEGER;  -- non-zero index to the last entry in the table
                              -- index to the table starts with 1

--
-- These are all indexes into the minvp arrays for the given pools
-- Indexes are not hardcoded
-- 
sp_idx BINARY_INTEGER;  -- Shared_Pool_size
jv_idx BINARY_INTEGER;  -- JaVa_pool_size
st_idx BINARY_INTEGER;  -- Sga_Target
cs_idx BINARY_INTEGER;  -- db_Cache_Size
pt_idx BINARY_INTEGER;  -- Pga_aggregate_Target
mt_idx BINARY_INTEGER;  -- Memory_Target
lp_idx BINARY_INTEGER;  -- Large_Pool_size
sr_idx BINARY_INTEGER;  -- StReams_pool_size
pl_idx BINARY_INTEGER;  -- Pga_aggregate_Limit
...
-- these size constants need to be in NUMBER and end with .0 to avoid
-- "ORA-01426: numeric overflow" when storing numbers like 2048*c_mb
c_kb           CONSTANT NUMBER := 1024.0;       -- 1 KB
c_mb           CONSTANT NUMBER := 1048576.0;    -- 1 MB
c_gb           CONSTANT NUMBER := 1073741824.0; -- 1 GB
...
db_cpus             NUMBER;     -- # of cpus
db_cpu_threads      NUMBER;     -- Threads per CPU

db_npdbs            NUMBER;     -- # of pdbs as seen in v$pdbs
...
PROCEDURE find_sga_mem_values (minvp  IN OUT MINVALUE_TABLE_T,
                               dbbit  NUMBER);
PROCEDURE init_mem_sizes (minvp      IN OUT MINVALUE_TABLE_T,
                          dbbit      IN     NUMBER,
                          max_minvp  IN OUT BINARY_INTEGER);
...
BEGIN
  ...
  db_n_version := 0;  -- initialize to 0 so that comparisons will be valid
                      -- if preupgrade tool is run on - for example - 12.2
  ...
  -- Find values for memory initialization parameters with minimum values
  -- Convert to numeric values
  store_oldval(minvp_db32);
  store_oldval(minvp_db64);

  -- determine new values for memory init parameters with minimum values
...
BEGIN
  ...
  -- check if java mitigation patch is installed and JAVA development is DISABLED
  define_check (i, 'JVM_MITIGATION_PATCH', c_check_level_warning,
                'Check if JAVA mitigation patch is ENABLED.');
  check_table(i-1).fix_type := c_fix_source_manual;
  ...
  define_check (i, 'CONC_RES_MGR', c_check_level_error,
                'Check if Concurrent statistics is properly set');
  check_table(i-1).valid_versions := '112,121,122';
  ...
  -- recomend apply PSU  if java mitigation patch is installed and JAVA development is DISABLED after upgrade
  define_check (i, 'POST_JVM_MITIGAT_PATCH', c_check_level_info,
                'Warn customer about JAVA mitigation patch is ENABLED at the end of the upgrade.');
  check_table(i-1).fix_type := c_fix_target_manual_post;
  
   -- Patch APEX to at least version 5.0.4
  define_check (i, 'APEX_VERSION_5', c_check_level_info,
                'If version of APEX is 5.0.1, 5.0.2 or 5.0.3 ask the user to patch it to at least version 5.0.4.');
  check_table(i-1).fix_type := c_fix_source_manual;
...
BEGIN
  ...
  --
  -- Initialize memory values for upgrade with minimum values
  --
  init_mem_sizes(minvp_db32, 32, max_minvp);
  init_mem_sizes(minvp_db64, 64, max_minvp);
  idx := max_minvp;  -- set local variable idx to the last index used in minvp
...
BEGIN
  ...
  IF db_n_version not in (102) AND  minvp(mt_idx).oldvalue != 0 THEN 
    ...
    FOR i IN 1..max_minvp LOOP
      IF i NOT IN (mt_idx,st_idx,pt_idx,sp_idx,jv_idx,cs_idx,lp_idx,sr_idx) AND
          (minvp(i).oldvalue IS NULL OR
          minvp(i).oldvalue < minvp(i).minvalue)
      THEN
  ...
  ELSIF minvp(st_idx).oldvalue != 0 THEN  -- SGA_TARGET in use
    find_sga_mem_values(minvp, dbbit);

    IF minvp(st_idx).newvalue > minvp(st_idx).oldvalue THEN
      minvp(st_idx).display := TRUE;
    END IF;
    ...
    FOR i IN 1..max_minvp LOOP
      IF i NOT IN (st_idx,mt_idx,cs_idx,jv_idx,sp_idx,lp_idx,sr_idx) AND 
          (minvp(i).oldvalue IS NULL OR
          minvp(i).oldvalue < minvp(i).minvalue)
      THEN
        minvp(i).display := TRUE;
  ...
  ELSE -- only pool sizes are used
    FOR i IN 1..max_minvp LOOP
      ...
      IF i NOT IN (st_idx,mt_idx,lp_idx,sr_idx) AND 
          (minvp(i).oldvalue IS NULL OR
           minvp(i).oldvalue < minvp(i).minvalue)
      THEN
        minvp(i).display := TRUE;
  ...
  minvp(lp_idx).display := FALSE;  -- large pool
  minvp(sr_idx).display := FALSE;  -- streams pool
  minvp(pt_idx).display := FALSE;  -- pga aggregate target

  -- do not recommend pga aggregate limit on upgrades to 12.1.0.2 - period
  -- bug 24696626
  minvp(pl_idx).display := FALSE;

END find_newval;

--------------------------- init_mem_sizes -------------------------------
-- This is called from init_initparams
...
IS
  ...
  batches     NUMBER;  -- approximate # of pdb upgrade "cycles"
  idx         BINARY_INTEGER;   -- index to minvp
BEGIN
  
  extra := 0;
  idx := 0;

  -- create minvp records and initialize the values to 0MB
  ...
  store_minval_dbbit(dbbit,idx,'db_cache_size', 0);
  cs_idx := idx;
  store_minval_dbbit(dbbit,idx,'java_pool_size', 0);
  jv_idx := idx;
  store_minval_dbbit(dbbit,idx,'shared_pool_size', 0);
  sp_idx := idx;  
  store_minval_dbbit(dbbit,idx,'large_pool_size', 0);
  lp_idx := idx;
  store_minval_dbbit(dbbit,idx,'streams_pool_size', 0);
  sr_idx := idx;
  store_minval_dbbit(dbbit,idx,'pga_aggregate_target', 0);
  pt_idx := idx;
  store_minval_dbbit(dbbit,idx,'pga_aggregate_limit', 0);
  pl_idx := idx;
  store_minval_dbbit(dbbit,idx,'sga_target', 0);
  st_idx := idx;
  store_minval_dbbit(dbbit,idx,'memory_target', 0);
  mt_idx := idx;
  max_minvp := idx;

  -- point to the right table of mem values
  IF dbbit = 32 THEN
    minvp := minvp_db32;
  ELSE
    minvp := minvp_db64;
  ...
  -- 12c doesn't support 32 bit database so there is no need to size
  -- for 32 bit database here.
  -- but since the data structures to support 32 bit db are already here
  -- and dbua in 12.1 is still using this info, we will keep the 32 bit code.
  -- but will not display the 32 bit values in manual preupgrade output
  -- and will remove the 32 bit code in 12.2.

  -- lets just make 32 bit values here the same as 64 bit values

  -- db_cache_size
  IF db_cpus <= 12 THEN
    minvp(cs_idx).minvalue := 48 * c_mb;
  ELSE
    minvp(cs_idx).minvalue := (4 * db_cpus) * c_mb;
  END IF;

  -- large pool size
  minvp(lp_idx).minvalue := 8 * c_mb;

  -- streams_pool_size
  minvp(sr_idx).minvalue := 0;  -- 0M
  ...
  IF sys.dbms_preup.is_db_noncdb = TRUE THEN  -- this db is a non-cdb
    ...
    extra := (264 * 1.5) * c_mb;
    minvp(sp_idx).minvalue := 264 * c_mb + extra;
    extra := 0;

    -- pga_aggregate_target
    minvp(pt_idx).minvalue := 384 * c_mb;

    -- pga_aggregate_limit (pl_idx)
    -- bug 22960672: recommend pga_aggregate_limit only for cdbs because
    --               we have not seen a non-cdb exceeding this parameter yet
    --               note: pga_aggregate_limit is a new parameter in 12.1.0.1
    minvp(pl_idx).display := FALSE;

  ELSE  -- IF db is a cdb
    db_npdbs := sys.dbms_preup.get_npdbs;
    ...
    IF db_npdbs >= trunc(db_cpus/2) THEN  -- if # of pdbs >= (# of cpus/2)
      IF db_cpus = 1 THEN
    ...
    batches := ceil(db_npdbs/pdbs_para);  -- estimated # of pdb upgrade cycles

    -- java pool needed is 64M per pdb upgrading in a batch
    extra := 50 * c_mb;
    minvp(jv_idx).minvalue := (64 * pdbs_para) * c_mb + extra;
    extra := 0;

    minvp(sp_idx).minvalue := 
      (  (264 * 3)
       + (264 * 2)
       + (pdbs_para * 200)
       + (db_npdbs * 20)
      ...
      --   constant in con id 1 
      -- + constant/reserve in con id 0 
      -- + reserve 200M of shared pool during pdb upgrade in pdb con id
      -- + up to 20m of misc in pdb con id that has been upgraded or opened
      -- + extra 

   -- pga_aggregate_target
    minvp(pt_idx).minvalue := 1152 * c_mb;

    -- pga_aggregate_limit
    minvp(pl_idx).minvalue := 3072 * c_mb;

  END IF;  -- IF db is a cdb

  minvp(st_idx).minvalue :=
    minvp(cs_idx).minvalue + minvp(jv_idx).minvalue +
    minvp(sp_idx).minvalue + minvp(lp_idx).minvalue +
    minvp(sr_idx).minvalue;

  minvp(mt_idx).minvalue :=
    minvp(cs_idx).minvalue + minvp(jv_idx).minvalue +
    minvp(sp_idx).minvalue + minvp(lp_idx).minvalue +
    minvp(sr_idx).minvalue + minvp(pt_idx).minvalue;
...
BEGIN
  ...
  -- streams pool (sr)
  IF minvp(sr_idx).oldvalue > minvp(sr_idx).minvalue THEN
    minvp(sr_idx).diff :=
      minvp(sr_idx).oldvalue - minvp(sr_idx).minvalue;
  END IF;

  -- pga_aggregate_target (pt)
  IF minvp(pt_idx).oldvalue > minvp(pt_idx).minvalue THEN
    minvp(pt_idx).diff :=
      minvp(pt_idx).oldvalue - minvp(pt_idx).minvalue;
  END IF;

  -- calculate sga_target 'newvalue' (new derived minimum) based on
  -- st_idx.minvalue and user-specified pool sizes.
  ...
  minvp(st_idx).newvalue := 
      minvp(st_idx).minvalue + minvp(cs_idx).diff
      + minvp(jv_idx).diff + minvp(sp_idx).diff
      + minvp(lp_idx).diff + minvp(sr_idx).diff
      + (128 * c_mb);
  ...
  minvp(mt_idx).newvalue :=
    ...
    + minvp(jv_idx).diff + minvp(sp_idx).diff
    + minvp(lp_idx).diff + minvp(sr_idx).diff
    + minvp(pt_idx).diff + (256 * c_mb);

  IF (minvp(st_idx).oldvalue != 0) THEN -- SGA_TARGET in use
    ...
    mtgval := minvp(st_idx).oldvalue + minvp(pt_idx).oldvalue + 128*c_mb;
           ...
           IF NOT (bChangesReq) THEN
             IF bDisplayStartBanner THEN
                 DisplayCenter('[Update Oracle Database ' || db_version ||
                              ' init.ora or spfile]');
                DisplayLine('[The minimum memory/pool size recommendations are for the upgrade process');
                 DisplayLine(' only.  After the upgrade, these values can be reverted or adjusted as');
                 DisplayLine(' applicable for the target release and production work.]');
             END IF;
             DisplayLine(' ');
...
BEGIN
  ...
  ELSE
    ...
    --
    -- 12c database only supports 64 bit so no need to display the 32 bit
    -- values for manual upgrades.
    -- output_manual_initparams(minvp_db32, FALSE);
    
    -- do not display [Update parameters] if the db is a pdb
    IF (dbms_preup.is_db_noncdb OR dbms_preup.is_con_root) THEN
      output_manual_initparams(minvp_db64, TRUE);
      -- the parameters currenly under this section are for memory and
      -- processes.  all of the parameters in this section - in 12.1.0.1
      -- anyway - are still cdb level parameters 
    END IF;
...
BEGIN
  IF HelpType = c_help_overview THEN
    return 'INFORMATION: --> Older Timezone in use' || crlf 
      ...
      || crlf || '     Please refer to My Oracle Support note number 1509653.1 for details.';
  ELSIF HelpType = c_help_fixup THEN
...
BEGIN
  IF HelpType = c_help_overview THEN
    result_txt := 'INFORMATION: --> ORDSYS.OrdImageIndex in use' 
...
BEGIN
  ...
  END;
   ...
   IF tmp_varchar1 IS NOT NULL OR tmp_varchar1 != '' THEN
      ...
      and a.status=''VALID'''
      INTO t_count;
      -- no local archive destination explicitly defined
      IF t_count = 0 THEN
         ...
         IF t_count > 0 THEN
            ...
            status := 1;
            END IF;
         END IF; -- standby archive destination in #1
      END IF; -- no local archive destination defined 
...
BEGIN
    ...
    and status='ENABLED';
    IF pOutputType = c_output_xml THEN
      result_txt:= genDBUAXMLCheck('JVM_MITIGATION_PATCH',
        c_check_level_warning,
        'The temporary OJVM Mitigation patch is currently installed and active in the database.',
        'The temporary OJVM Mitigation patch is currently installed and active in the database, '||
        'securing it against the known threat but disabling Java development until you can install '|| 
        'the latest OJVM PSU.',
        'If you plan on continuing to use the Oracle mitigation patch instead of installing the Oracle recommended'||
		...
		'Taking this action will make your database subject to a known security vulnerability, but without the PSU patch'||
		'Java development must still be enabled to allow some upgrade actions to complete. Extreme caution is advised.',
        c_dbua_detail_type_text,
        'The upgrade may fail if Java development is disabled. When active, the OJVM Mitigation patch disables Java development '||
		'in the database, Oracle strongly recommends installing the latest OJVM PSU in both '|| db_version|| ' and ' ||
		c_version||' ORACLE_HOMEs which will eliminate the need for the mitigation patch. Doing that in '|| c_version||
	        ' will not incur in any database downtime and will eliminate the same vulnerability during the next upgrade. '||
		'If you cannot apply the OJVM PSU, you may opt to retain the mitigation patch in '|| db_version||', but Java development '||
                'still must be enabled during the upgrade. '||
                'Please refer to MOS note 1929745.1 for more details.',
        c_dbua_fixup_type_manual,
        c_dbua_fixup_stage_pre);
    ELSE
      result_txt := jvm_mitigation_patch_gethelp(c_help_overview);
    END IF;
    RETURN c_status_failure;
EXCEPTION
  WHEN NO_DATA_FOUND THEN RETURN c_status_success;
...
BEGIN
  IF HelpType = c_help_overview THEN
    return 'WARNING: --> The temporary OJVM Mitigation patch is currently installed and active '
           || crlf || '     in the database, securing it against the known threat but disabling JAVA '
           || crlf || '     development until you can install the latest OJVM PSU.'||crlf
           || crlf || '     The upgrade may fail if Java development is disabled.'  
           || crlf || '     When active, the OJVM Mitigation patch disables Java development '  
           || crlf || '     in the database, Oracle strongly recommends installing the latest ' 
           || crlf || '     OJVM PSU in both '|| db_version||' and '||c_version||' ORACLE_HOMEs which will ' 
           || crlf || '     eliminate the need for the mitigation patch. Doing that in '|| c_version
           || crlf || '     will not incur in any database downtime and will eliminate the same ' 
           || crlf || '     vulnerability during the next upgrade.'||crlf
           || crlf || '     If you cannot apply the OJVM PSU, you may opt to retain the mitigation patch' 
           || crlf || '     in '|| db_version||', but Java development still must be enabled during '  
           || crlf || '     the upgrade. Please refer to MOS note 1929745.1 for more details.'||crlf
           || crlf || '     If you plan on continuing to use the Oracle mitigation patch instead of '
           ...
           || crlf || '     Taking this action will make your database subject to a known security vulnerability, ' 
           || crlf || '     but without the PSU patch, Java development must still be enabled to allow some'
           || crlf || '     upgrade actions to complete. Extreme caution is advised.';
  ELSIF HelpType = c_help_fixup THEN
    return 'Enable JAVA development prior to the database upgrade by executing command: EXECUTE SYS.DBMS_JAVA_DEV.ENABLE;';
  END IF;
END;
...
BEGIN
  -- Call fixup and throw away the result
  status := jvm_mitigation_patch_fixup (result, tSqlcode);
...
BEGIN
    ...
    and status='ENABLED';
    IF pOutputType = c_output_xml THEN
      result_txt:= genDBUAXMLCheck('POST_JVM_MITIGAT_PATCH',
        c_check_level_warning,
        'The temporary OJVM Mitigation patch is currently installed and active in the database.',
        'The temporary OJVM Mitigation patch is currently installed and active in the database, '||
        'securing it against the known threat but disabling Java development until you can install '|| 
        'the latest OJVM PSU.',
        'After upgrade, apply latest Oracle JavaVM (OJVM) Bundle Patch (BP) or '||
        ...
        c_dbua_fixup_stage_post);
    ELSE
      result_txt := post_jvm_mitigat_patch_gethelp(c_help_overview);
    END IF;
    RETURN c_status_failure;
EXCEPTION
  WHEN NO_DATA_FOUND THEN RETURN c_status_success;
...
BEGIN
  IF HelpType = c_help_overview THEN
    return 'INFORMATION: --> The temporary OJVM Mitigation patch is currently installed and active'
           || crlf || '     in the database, securing it against the known threat but disabling JAVA'
           || crlf || '     development until you can install the latest OJVM PSU.'||crlf
           || crlf || '     Oracle JavaVM Component Database PSU is released as part of the'
...
BEGIN
  -- Call fixup and throw away the result
  status := post_jvm_mitigat_patch_fixup (result, tSqlcode);
END post_jvm_mitigat_patch_fixup;

--
-- Fixup post_jvm_mitigat_patch
--
FUNCTION post_jvm_mitigat_patch_fixup (
         result_txt IN OUT VARCHAR2,
         pSqlcode    IN OUT NUMBER) RETURN NUMBER
IS
BEGIN
...
IS
    ...
    v_conc_on_text varchar2(250);
    v_res_man_on_text varchar2(250);
    disable_conc_stats_text varchar2(75) := 'DBMS_STATS.SET_GLOBAL_PREFS(''CONCURRENT'',''FALSE'')';
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            return c_status_success;  -- CONCURRENT STATS ARE DISABLED so we do not need to check anything further.
        END IF;
 ...
 --
    ...
    IF (v_is_conc_on_boolean AND NOT v_is_res_man_on_boolean) THEN
        IF pOutputType = c_output_xml THEN
          result_txt:= genDBUAXMLCheck('CONC_RES_MGR',
            c_check_level_warning,
            'The CONCURRENT statistics preference is enabled. 
            However, Resource Manager is disabled.',
            'The CONCURRENT statistics preference is enabled. 
            However, Resource Manager is disabled.',
            'If the concurrent statistics preference is set to 
            any value other than OFF, then Resource Manager must be enabled.'
            ||  'The concurrent statistics must be changed in 
            DBMS_STATS.SET_GLOBAL_PREFS(). ' ,
             c_dbua_detail_type_text,
            'Disable concurrent statistics gathering by setting the 
            CONCURRENT statistics preference to OFF or enable Resource Manager.' 
            ||'To disable the CONCURRENT statistics preference using:' 
            ||'execute ' || disable_conc_stats_text
            ||' For further information see My Oracle Support Note 2049013.1.',
            c_dbua_fixup_type_auto,
            c_dbua_fixup_stage_pre );
        ELSE
          result_txt :=  CONC_RES_MGR_gethelp(c_help_overview);
        END IF;
        IF pOutputFixupScripts THEN
          genFixup ('CONC_RES_MGR');
        END IF;
        RETURN c_status_failure;
    END IF;
...
FUNCTION CONC_RES_MGR_gethelp (HelpType IN NUMBER) RETURN VARCHAR2
IS
    disable_conc_stats_text varchar2(250) := 
    	'execute DBMS_STATS.SET_GLOBAL_PREFS	(''CONCURRENT'',''FALSE'')';  
    --This will happen only in 11.2 databases
BEGIN
  IF HelpType = c_help_overview THEN
    return 'WARNING: --> The CONCURRENT statistics preference is enabled.'
    ||crlf||'      However, Resource Manager is disabled, which is an invalid '
    ||crlf||'      configuration for gathering statistics concurrently. Disable'
    ||crlf||'      concurrent statistics gathering by setting the CONCURRENT statistics '
    ||crlf||'      preference to OFF or enable Resource Manager.';
  ELSIF HelpType = c_help_fixup THEN
    return disable_conc_stats_text;
  END IF;
END CONC_RES_MGR_gethelp;
--
-- Fixup (Procedure and function)
--
PROCEDURE CONC_RES_MGR_fixup
...
BEGIN
  -- Call fixup and throw away the result
  status := CONC_RES_MGR_fixup (result, tSqlcode);
END CONC_RES_MGR_fixup;

FUNCTION CONC_RES_MGR_fixup (
         result_txt IN OUT VARCHAR2,
         pSqlcode    IN OUT NUMBER) RETURN number
IS
BEGIN
  result_txt := CONC_RES_MGR_gethelp(c_help_overview);
  pSqlcode := 0;
  return execute_sql_statement ('begin DBMS_STATS.SET_GLOBAL_PREFS(''CONCURRENT'',''FALSE''); end;', result_txt, pSqlcode);
END CONC_RES_MGR_fixup;
--
-- end of CONC_RES_MGR
--
...
FUNCTION  apex_version_5_check (result_txt OUT VARCHAR2) RETURN number
IS
	apex_version VARCHAR2(30);
BEGIN
	BEGIN
		EXECUTE IMMEDIATE 'SELECT VERSION FROM DBA_REGISTRY 
       	WHERE COMP_ID=''APEX'''
		INTO apex_version;
		EXCEPTION WHEN no_data_found THEN
		apex_version := '';
	END;
	IF (apex_version LIKE '5.0.1%' OR apex_version LIKE '5.0.2%'
	OR apex_version LIKE '5.0.3%') THEN
	--APEX is in one of the not supported versions
	        IF pOutputType = c_output_xml THEN
			result_txt:= genDBUAXMLCheck('APEX_VERSION_5',
			c_check_level_info,
			'APEX in your database is at version ' || apex_version,
			'APEX in your database is at version ' || apex_version,
			'Patch APEX component to at least version 5.0.4 prior to upgrading the database. If patch '
                        || 'is not applied, then after database upgrade refer to MOS note 2121218.1 for details on '
                        || 'how to solve the validation error.',
			c_dbua_detail_type_text,
			'If Oracle Application Express 5.0.1, 5.0.2, or 5.0.3 is installed, then attempting to '
                        || 'validate APEX can generate ''ORA-20001: Error invalidation: ORA-01919: role ''INHERIT '
                        || 'ANY PRIVILEGES'' does not exist''.',
			c_dbua_fixup_type_manual,
			c_dbua_fixup_stage_pre);
		ELSE	
			result_txt := apex_version_5_gethelp(c_help_overview);
		END IF;
		IF pOutputFixupScripts THEN
			genFixup ('APEX_VERSION_5');
		END IF;
		RETURN c_status_failure;
	ELSE
	 	-- APEX is in the desired version
		RETURN c_status_success;
	END IF;
END apex_version_5_check; 
...
BEGIN
  IF HelpType = c_help_overview THEN
    EXECUTE IMMEDIATE 'SELECT VERSION FROM DBA_REGISTRY
        WHERE COMP_ID=''APEX'''
        INTO apex_version;
    return 'INFORMATION: --> APEX in your database is at version ' || apex_version || crlf
     || crlf || '     If Oracle Application Express 5.0.1, 5.0.2, or 5.0.3 is installed,'
     || crlf || '     then attempting to validate APEX can generate ''ORA-20001: Error' 
     || crlf || '     invalidation: ORA-01919: role ''INHERIT ANY PRIVILEGES'' does not exist.'    
     || crlf || '     Patch APEX component to at least version 5.0.4 prior to upgrading'
     || crlf || '     the database. If patch is not applied, then after database upgrade'
     || crlf || '     refer to MOS note 2121218.1 for details on how to solve the'
     || crlf || '     validation error.';
  ELSIF HelpType = c_help_fixup THEN
    return 'Refer to MOS note 2121218.1 for details';
  END IF;
END apex_version_5_gethelp;
...
BEGIN
  -- Call fixup and throw away the result
  status := apex_version_5_fixup (result, tSqlcode);
END apex_version_5_fixup;

FUNCTION  apex_version_5_fixup (
	  result_txt IN OUT VARCHAR2, 
	  pSqlcode IN OUT NUMBER) RETURN number
IS
BEGIN
