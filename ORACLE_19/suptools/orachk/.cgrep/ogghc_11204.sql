...
prompt
---- Begin Check for Trace Events
create or replace procedure check_event(event_num  IN number,info_exists IN varchar2,is_error IN boolean := FALSE,cnt IN OUT NUMBER)
as
  prefix           varchar2(25);
  event_level number;
begin
  ...
  event_level := 0;
  sys.dbms_system.read_ev(event_num,event_level);
    if (event_level > 0) then
      dbms_output.put_line(prefix ||info_exists ||' Event '||to_char(event_num)||' set at level '||
        to_char(event_level)||'.  To disable, use ALTER SYSTEM SET EVENTS='''||to_char(event_num)||' trace name context off;''');
    cnt := cnt+1;
    end if;
end;
/
...
begin
   ...
   check_event(1348,'Logminer',false,a);
   check_event(1349,'Logminer',false,a);

   check_event(26700,'OGG Background Server',false,a);

   if a <> 0 then
     dbms_output.put_line('Please disable the above tracing if it is no longer needed.');
   end if;

   
end;
/
---- End Check for Trace Events
