package mineocr;
# 
# $Header: tfa/src/orachk_py/scripts/mineocr.pm /main/6 2018/07/05 04:32:42 rojuyal Exp $
#
# mineocr.pm
# 
...
#    cnagur      04/22/16 - Fixed issues in getGridHome
#    rojuyal     11/24/15 - Creation
# 
...
sub get_oratab {
    ...
    my ($platform) 	= get_platform();
    if ($platform eq "solaris" || $platform eq "SunOS") { $oratab = '/var/opt/oracle/oratab'; }
...
sub get_olrloc {
    ...
    my ($platform) 	= get_platform();
    if ($platform eq "solaris" || $platform eq "SunOS") { $olrloc = '/var/opt/oracle/olr.loc'; }
...
sub discover_crs_db_asm_and_write_kvout {
	...
	my (@nodes);
        ...
        foreach my $okey1(keys %res_1) {
    	    ...
    	    #} elsif ($okey1 =~ m/\.asm$/i) {
            } elsif ($okey1 =~ m/\.asm$/i or $okey1 =~ m/\.asm\(.*\.asmgroup\)$/i ) {
                foreach my $ikey1(keys %{$res_1{$okey1}}) {
            	    foreach my $i2key1(keys %{$res_1{$okey1}{$ikey1}}) {
            		if ($i2key1 =~ m/^STATE$/i) {
	...
	my (%rdbms_oh);
        foreach my $okey2(keys %res_2) {
            if ($okey2 =~ m/\.db$/i) {
    	        my (@nodes);
    	        my (%nimap);
    	        my ($dbname,$dbhome,$dbowner,$VERSION,$dbversion,$dbrole,$dbgn,$dbvariant);
    	        $VERSION = "";
    	        $dbversion = "";
                ...
                foreach my $ikey2(keys %{$res_2{$okey2}}) {
	        	    ...
	        	    $dbversion	= $VERSION;
                        ...
                        } elsif ($i2key2 =~ m/^DATABASE_TYPE/i) {
                        	$dbvariant = $res_2{$okey2}{$ikey2}{$i2key2};
    	        	}
                    }
		    ...
		    if (exists $rdbms_oh{$mstr}) {
    		    ...
    		    if ($pdebug == 1) { print Dumper(\%nimap); }
		    ...
		    if (grep {$_ =~ /$dbname:/i && $dbistate{$_} =~ /online/i} keys %dbistate) {
	                ...
	                if (defined $dbvariant && $dbvariant =~ /SINGLE/i) {
	                	$dbvariant = 'SingleInstance'
	                } elsif (defined $dbvariant && $dbvariant =~ /RAC/i) {
	                	$dbvariant = 'RAC'
	                }
	                print CFIL "$dbname.DATABASE_VARIANT = $dbvariant\n";
		    } else {
			...
			print CFIL "$dbname.RUNNING = 1\n";
		    }
                }
            #} elsif ($okey2 =~ m/\.asm$/i) {    
    	    } elsif ($okey2 =~ m/\.asm$/i or $okey2 =~ m/\.asm\(.*\.asmgroup\)$/i ) {
    	        my (@nodes);
    	        my (%nimap);
    	        my ($VERSION) = "";
	...
	my $INV_LOC;
	if (defined $CRS_HOME) {
	  $INV_LOC = File::Spec->catfile("$CRS_HOME", "inventory", "ContentsXML", "comps.xml");
	}
        ...
        if (defined $crs112 && $crs112 >= 1) {
            ...
            print CFIL "ASM_INVENTORY = $INV_LOC\n";
        }
        print CFIL "CRS_HOME = $CRS_HOME\n";
        print CFIL "CRS_INVENTORY = $INV_LOC\n";
        close(CFIL);
    }
    return $CRS12;
}
